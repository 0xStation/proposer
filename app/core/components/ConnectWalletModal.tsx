...
import Marble from "/public/marble-no-background.svg"
...
import { marbleConnector } from "app/utils/marble"
import { LoginResultState } from "@marblexyz/wagmi-connector"
          ...
          <div className="mx-6 mb-6 flex flex-col">
            ...
            <div className="flex-1 mx-auto w-full flex flex-col">
              <p className="text-lg text-center mb-6">
                New to web3? Create a wallet using your email below:
              </p>
              <button
                className="mx-auto border border-marble-white rounded-md content-center hover:bg-wet-concrete cursor:pointer w-80 sm:mt-0 mt-2 h-[35px]"
                disabled={connectState.loading}
                onClick={async () => {
                  trackClick(WALLET_CONNECTION.EVENT_NAME.WALLET_CONNECTION_BUTTON_CLICKED, {
                    pageName: window.location.href,
              ...
              >
                {accountData?.isConnecting && metamaskWallet?.id === pendingConnector?.id ? (
                  ...
                  <div className="flex flex-row justify-center items-center space-x-2 my-1">
                    <div className="flex-3/5">
                      <span>{`Log in or sign up using Marble`}</span>
                    </div>
                    <div className="flex flex-2/5 justify-center items-center">
                      <Image src={Marble} alt="Marble logo." width={21} height={21} />
                    </div>
                  </div>
                )}
              </button>
