...
export const ApproveGnosisTransactionModal = ({ isOpen, setIsOpen, milestone, nonce }) => {
  const setToastState = useStore((state) => state.setToastState)
  const activePayment = milestone.payments[0]

  const { signMessage: signGnosis } = useGnosisSignatureToConfirmTransaction(activePayment, nonce)
