...
import createPayment from "app/proposalPayment/mutations/createPayment"
...
export const ReQueueGnosisTransactionModal = ({
  isOpen,
  setIsOpen,
  milestone,
  originalPayment,
  ...
  // it's okay to be signing the original payment instead of the new payment record...
  // the original payment serves as a "record" and has the same information
  const { signMessage: signGnosis } = useGnosisSignature(originalPayment)
  ...
  const QueuePaymentTab = () => {
    return (
      <>
        <h3 className="text-2xl font-bold mt-4">Re-queue transaction</h3>
        <p className="mt-2">
          Sign to re-queue this transaction on Gnosis. Afterwards, you and other signers will be
          able to view and execute this transaction on the Gnosis app.
        </p>
  ...
  const AttachPaymentTab = () => {
    const chainId = originalPayment.data.token.chainId
    return (
      <>
  ...
  return (
    <Modal open={isOpen} toggle={setIsOpen}>
      <div className="p-2">
        {!activeChain || activeChain.id !== originalPayment?.data?.token.chainId ? (
          <SwitchNetworkView
            isLoading={isLoading}
            setIsLoading={setIsLoading}
            chainId={originalPayment?.data?.token.chainId}
          />
        ) : (
          <>
...
export default ReQueueGnosisTransactionModal
